...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    const acc = rows[0];
    if (!acc) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Account not found' }); }
    if (acc.type === 'crypto') { await client.query('ROLLBACK'); return res.status(400).json({ error: 'Cannot send from crypto account' }); }
    if (parseFloat(acc.balance) < amount) { await client.query('ROLLBACK'); return res.status(422).json({ error: 'Insufficient funds' }); }
